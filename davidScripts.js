var plotPatients = function(data_in) {
    ...
    var pathP = d3.select("svg#patientdisp")
        .selectAll("path")
        .data(data_in)
        .enter()
        .append("path");
    ...
    var changept3 = function() {
        pathP = d3.select("svg#patientdisp")
            .data(data_in)
    ...
    d3.select("#chemo1").on("change", changept);
    var highlightOrder = false;
    
    d3.select("#died").on("change", changept2);
    var highlightOrder2 = false;
    
    d3.select("#survivalTime").on("change", changept3);
